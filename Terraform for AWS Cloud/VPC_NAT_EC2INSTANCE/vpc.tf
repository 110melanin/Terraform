...
resource "aws_subnet" "terra-subnet-public-3" {
  vpc_id                  = aws_vpc.terra-vpc.id
...
# (Subnet 3)
resource "aws_subnet" "terra-subnet-private-3" {
  vpc_id                  = aws_vpc.terra-vpc.id
...
resource "aws_route_table_association" "terra-public-rt-3a" {
  subnet_id      = aws_subnet.terra-subnet-public-3.id
  route_table_id = aws_route_table.terra-public-rt.id
}